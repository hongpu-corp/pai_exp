...
def threadWork(resource_id_list, version_id, headers, path):
    imgNumber = len(resource_id_list)
    index = 0
    for resource_id in resource_id_list:
        ...
        for name in label:
            if name == '':
                ...
                vocwriter.save(targetFile=xml_path)
        index += 1
        logging.info('the total number of res is ' + str(imgNumber) + ',' + 'now we are downloading res ' + str(index))  # 正常信息
...
def download_train_res(project, version, path):  #项目 版本 本地保存路径
    url = 'http://vi-plat.chinaeast2.cloudapp.chinacloudapi.cn:8000/api/v1/auth/'
    data = json.dumps({'email': 'sys_admin', 'password': 'hongpu'})
    response = requests.post(url, data)  # 发出请求
    res = json.loads(response.text)
    if res != -1 and res != 0:
        token = res['token']
    ...
    threadWork(resource_id_list, version_id, headers, path)
